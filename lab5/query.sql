...
WHERE
    h.name = 'Космос' AND
    rc.name = 'Люкс' AND
    rib.checkin_date <= '2019-04-01' AND
...
WHERE
    (rib.checkout_date < '2019-04-22' OR
    rib.checkin_date >= '2019-04-22') OR
    rib.id_room IS NULL;

# 4. Дать количество проживающих в гостинице “Космос” на 23 марта по каждой категории номеров
SELECT
    rc.id_room_category, COUNT(rib.id_room_in_booking) residents_count
FROM
    room_in_booking rib
    LEFT JOIN room r ON r.id_room = rib.id_room
...
WHERE
    h.name = 'Космос' AND
    rib.checkin_date <= '2019-03-23' AND
...
SELECT
    room_last_checkout_date.id_room, room_last_checkout_date.last_checkout_date, c.*
FROM
    (SELECT
        r.id_room, MAX(rib.checkout_date) last_checkout_date
    FROM
        room_in_booking rib
        LEFT JOIN room r ON r.id_room = rib.id_room
        LEFT JOIN booking b ON rib.id_booking = b.id_booking
        LEFT JOIN hotel h ON r.id_hotel = h.id_hotel
    WHERE
        MONTH(rib.checkout_date) = 4 AND
        h.name = 'Космос'
    GROUP BY
        r.id_room) room_last_checkout_date
...
WHERE
    rib.checkin_date = '2019-05-10' AND
    h.name = 'Космос' AND
    rc.name = 'Бизнес';
...
WHERE
    (rib.checkin_date BETWEEN rib2.checkin_date AND rib2.checkout_date) AND
    rib.id_booking != rib2.id_booking;
